...
class CssStyledContentController extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	public function render_table($content, $conf) {
		...
		if ($hookObj = $this->hookRequest('render_table')) {
			...
			$tableWidth = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'acctables_tablewidth');
			$tableColWidth = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'acctables_tablecolwidth');
			$tableRowContext = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'acctables_tablerowcontext');
			...
			$tableColWidthArr = str_getcsv($tableColWidth, $delimiter, $quotedInput);
			$tableRowContextArr  = str_getcsv($tableRowContext, $delimiter, $quotedInput);
			...
			foreach ($rows as $k => $v) {
				...
				for ($a = 0; $a < $cols; $a++) {
					...
					$style = !$k && !$noStyles && $tableColWidthArr[$a] ? ' style="width:'.$tableColWidthArr[$a].'%;"' : '';
					...
					if ($headerPos == 'top' && !$k || $headerPos == 'left' && !$a) {
						...
						$newCells[$a] = '
							<th' . $cellAttribs . $scope . $style .'>' . $this->cObj->stdWrap($cells[$a], $conf['innerStdWrap.']) . '</th>';
					} else {
						if (empty($headerPos)) {
							$accessibleHeader = '';
						} else {
						...
						$newCells[$a] = '
							<td' . $cellAttribs . $accessibleHeader . $style .'>' . $this->cObj->stdWrap($cells[$a], $conf['innerStdWrap.']) . '</td>';
					}
				}
				if (!$noStyles) {
					...
					if ($tableRowContextArr[$k]) {
						$rowAttribs = $k > 0 && $rCount - 1 == $k ? ' class="' . $tableRowContextArr[$k] .' '. $oddEven . ' tr-last"' : ' class="' . $tableRowContextArr[$k] .' '. $oddEven . ' tr-' . $k . '"';
					}
				}
				$rows[$k] = '
			...
			$tableStyle = $tableWidth && !$noStyles ? ' style="width:auto;"' : '';
			...
			$out = '
				<table ' . GeneralUtility::implodeAttributes($tableTagParams) . $tableStyle .'>' . $tableContents . '
				</table>';
			// Return value
			return $out;
		}
